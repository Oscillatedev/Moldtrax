...
namespace Moldtrax.Controllers
{
    ...
    public class DetailMoldInfoController : Controller
    {
        ...
        public ActionResult SearchMold(string str = "")
        {
            if (str == "")
            {
                var tb = db.TblMoldData.ToList().OrderBy(x => x.MoldName);
                return PartialView("_MoldData", tb);
            }
            else
            {
                var tb = db.TblMoldData.Where(x => x.MoldName.Contains(str) || x.MoldDesc.Contains(str)).ToList();
                return PartialView("_MoldData", tb);
            }
        }
        ...
        public ActionResult SortingMoldList(int ID = 0)
        {
            ...
            {
                var tb = db.TblMoldData.ToList().OrderByDescending(x => x.MoldName);
                return PartialView("_MoldData", tb);
            }
        }
